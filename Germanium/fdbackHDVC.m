...
@interface fdbackHDVC ()

@end

@implementation fdbackHDVC
@synthesize table;
...
- (void)viewWillAppear:(BOOL)animated 
{
	...
	toolbar.frame =  CGRectMake(0.0, 980.0, 768.0, 44.0);
	[self.navigationController.view addSubview:toolbar];
	
	sendBtn = [UIButton buttonWithType:UIButtonTypeCustom];
	sendBtn.frame = CGRectMake(365.0, 0.0, 106.0, 44.0);
    ...
    if (cell == nil) {
		...
		if([indexPath section] == 0)
		{
			if(!mailTxtField)
			{
				...
				CGRect frame = CGRectMake(5.0, 5.0, 670.0, 70);
		...
		else if([indexPath section] == 2)
		{
			...
			if(!messageTxtField)
			{
				CGRect frame = CGRectMake(5.0, 5.0, 670, 210.0);
